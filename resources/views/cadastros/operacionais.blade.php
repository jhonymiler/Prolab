        ...
        <div class="card">
            <div class="card-header border-bottom p-1">
                ...
                <div class="dt-action-buttons text-end">
                    @can('criar custos operacionais')
                        <div class="dt-buttons d-inline-flex">
                            <button class="dt-button create-new btn btn-primary" tabindex="0" id="new-operacionais"
                                type="button" data-bs-toggle="modal" data-bs-target="#operacionais"><span><i
                                        data-feather='plus'></i> Novo
                                    Custo Operacional</span></button>
                        </div>
                    @endcan
                ...
                <table id="tabela-dados" class="table" style="width:100%">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th>Status</th>
                            <th>Observação</th>
                            @canany(['editar custos operacionais', 'deletar custos operacionais'])
                                <th>Ações</th>
                            @endcanany
                        </tr>
                    </thead>
                        ...
                        @if ($dados)
                            @foreach ($dados as $m)
                                <tr>
                                    ...
                                    <td>{{ $m->observacao }}</td>
                                    @canany(['editar custos operacionais', 'editar custos operacionais'])
                                        <td>
                                            <div class="d-flex">
                                                @can('editar custos operacionais')
            ...
            @canany(['criar custos operacionais', 'editar custos operacionais'])
                <div class="modal fade" id="operacionais" tabindex="-1">
                    <div class="modal-dialog modal-dialog-centered modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Adicionar Custo Operacional</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <form id="form-operacionais" class="add-new-user modal-content pt-0" method="post"
                                action="{{ route('custo-operacional-save') }}">
                                @csrf
...
@section('page-script')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#new-operacionais").click(function() {
                form.attr('action', "{{ route('custo-operacional-save') }}").reset()
            });

            $(".editar").click(function() {
                json = $(this).data('json')
                form.attr('action', '/custo-operacional/save/' + json.id)
                for (var v in json) {
                    if (v == 'valor') {
            ...
            $(".ativar_desativar").click(function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        window.location.href = '/custo-operacional/ativa_desativa/' + id;
