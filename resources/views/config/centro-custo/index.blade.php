    ...
    <section class="app-user-list">
        ...
        <div class="card">
                ...
                <div class="dt-action-buttons text-end">
                    @can('criar configuracoes')
                        <div class="dt-buttons d-inline-flex">
                            <button class="dt-button create-new btn btn-primary" tabindex="0" id="new-centro-custo"
                                type="button" data-bs-toggle="modal" data-bs-target="#centro-custo"><span><i
                                        data-feather='plus'></i> Novo
                                    Centro de Custo</span></button>
                        </div>
                    @endcan
                        ...
                        @if ($cc)
                            @foreach ($cc as $c)
                                <tr>
                                    ...
                                    <td>{{ $c->nome }}</td>
                                    <td>{{ $c->created_at->format('d/m/Y H:m:i') }}</td>
                                    <td>{{ $c->updated_at->format('d/m/Y H:m:i') }}</td>
                                    <td>
                                        @if ($c->status == 1)
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                @can('editar configuracoes')
                                                    <button data-id="{{ $c->id }}" data-nome="{{ $c->nome }}"
                                                ...
                                                @can('deletar configuracoes')
                                                    @if ($c->status == 1)
                                                        <button data-id="{{ $c->id }}" data-status="{{ $c->status }}"
                                                            type="button"
            ...
            @canany(['criar configuracoes', 'editar configuracoes'])
                <div class="modal modal-slide-in new-user-modal fade" id="centro-custo">
                    <div class="modal-dialog">
                        <form id="form-centro-custo" class="add-new-user modal-content pt-0" method="post"
                            action="{{ route('centro-custo-save') }}">
                            @csrf
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">×</button>
                            <div class="modal-header mb-1">
                                <h5 class="modal-title" id="exampleModalLabel">Adicionar Centro de Custo</h5>
                            </div>
                            <div class="modal-body flex-grow-1">
                                <div class="mb-1">
                                    <label class="form-label" for="nome">Nome</label>
                                    <input type="text" class="form-control dt-full-name" id="nome"
                                        placeholder="Centro de Custo" name="nome" />
                                </div>
...
@section('page-script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#new-centro-custo").click(function() {
                form.attr('action', "{{ route('centro-custo-save') }}").reset()
            });

            $(".editar").click(function() {
                id = $(this).data('id')
                nome = $(this).data('nome')
                form.attr('action', '/config/centro-custo/' + id)
                $("#nome").val(nome);
                $("#centro-custo").modal('show')
            });
            ...
            $(".ativar_desativar").click(function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        window.location.href = '/config/centro-custo/ativa_desativa/' + id;
