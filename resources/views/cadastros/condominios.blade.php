...
@section('title', 'Custos Condominios')
        ...
        <div class="card">
            <div class="card-header border-bottom p-1">
                ...
                <div class="dt-action-buttons text-end">
                    @can('criar custos condominios')
                        <div class="dt-buttons d-inline-flex">
                            <button class="dt-button create-new btn btn-primary" tabindex="0" id="new-condominios"
                                type="button" data-bs-toggle="modal" data-bs-target="#condominios"><span><i
                                        data-feather='plus'></i> Novo
                                    Custo Condominio</span></button>
                        </div>
                    @endcan
                ...
                <table id="tabela-dados" class="table" style="width:100%">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th>Status</th>
                            <th>Observação</th>
                            @canany(['editar custos condominios', 'deletar custos condominios'])
                                <th>Ações</th>
                            @endcanany
                        </tr>
                    </thead>
                        ...
                        @if ($dados)
                            @foreach ($dados as $m)
                                <tr>
                                    ...
                                    <td>{{ $m->observacao }}</td>
                                    @canany(['editar custos condominios', 'editar custos condominios'])
                                        <td>
                                            <div class="d-flex">
                                                @can('editar custos condominios')
            ...
            @canany(['criar custos condominios', 'editar custos condominios'])
                <div class="modal fade" id="condominios" tabindex="-1">
                    <div class="modal-dialog modal-dialog-centered modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Adicionar Custo Condominio</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <form id="form-condominios" class="add-new-user modal-content pt-0" method="post"
                                action="{{ route('custo-condominio-save') }}">
                                @csrf
                                ...
                                <div class="modal-body flex-grow-1">
                                    <div class="row">
                                        <div class="col-12 col-md-6">
                                            <div class="mb-1">
                                                <label class="form-label" for="nome">Descrição do Custo</label>
...
@section('page-script')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#new-condominios").click(function() {
                form.attr('action', "{{ route('custo-condominio-save') }}").reset()
            });

            $(".editar").click(function() {
                json = $(this).data('json')
                form.attr('action', '/custo-condominio/save/' + json.id)
                for (var v in json) {
                    if (v == 'valor') {
            ...
            $(".ativar_desativar").click(function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        window.location.href = '/custo-condominio/ativa_desativa/' + id;
