...
@section('title', 'Cadastro de Fundações')
        ...
        <div class="card">
                ...
                <div class="dt-action-buttons text-end">
                    @can('criar configuracoes')
                        <div class="dt-buttons d-inline-flex">
                            <button class="dt-button create-new btn btn-primary" tabindex="0" id="new-fundacao" type="button"
                                data-bs-toggle="modal" data-bs-target="#fundacao"><span><i data-feather='plus'></i> Novo
                                    Fundação</span></button>
                        </div>
                    @endcan
            ...
            <div class="card-datatable table-responsive pt-0">
                <table id="tabela-dados" class="table" style="width:100%">
                    ...
                    <tbody>
                        @if ($fund)
                            @foreach ($fund as $fundacao)
                                <tr>
                                    <td>{{ $fundacao->id }}</td>

                                    <td>{{ $fundacao->nome }}</td>
                                    <td>{{ $fundacao->created_at->format('d/m/Y H:m:i') }}</td>
                                    <td>{{ $fundacao->updated_at->format('d/m/Y H:m:i') }}</td>
                                    <td>
                                        @if ($fundacao->status == 1)
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                @can('editar configuracoes')
                                                    <button data-id="{{ $fundacao->id }}" data-nome="{{ $fundacao->nome }}"
                                                ...
                                                @can('deletar configuracoes')
                                                    @if ($fundacao->status == 1)
                                                        <button data-id="{{ $fundacao->id }}"
                                                            data-status="{{ $fundacao->status }}" type="button"
            ...
            @canany(['criar configuracoes', 'editar configuracoes'])
                <div class="modal modal-slide-in new-user-modal fade" id="fundacao">
                    <div class="modal-dialog">
                        <form id="form-fundacao" class="add-new-user modal-content pt-0" method="post"
                            action="{{ route('fundacao-save') }}">
                            @csrf
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">×</button>
                            <div class="modal-header mb-1">
                                <h5 class="modal-title" id="exampleModalLabel">Adicionar Fundação</h5>
                            </div>
                            <div class="modal-body flex-grow-1">
                                <div class="mb-1">
                                    <label class="form-label" for="nome">Nome</label>
                                    <input type="text" class="form-control dt-full-name" id="nome"
                                        placeholder="Fundação" name="nome" />
                                </div>
...
@section('page-script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#new-fundacao").click(function() {
                form.attr('action', "{{ route('fundacao-save') }}").reset()
            });

            $(".editar").click(function() {
                id = $(this).data('id')
                nome = $(this).data('nome')
                form.attr('action', '/config/fundacao/' + id)
                $("#nome").val(nome);
                $("#fundacao").modal('show')
            });
            ...
            $(".ativar_desativar").click(function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        window.location.href = '/config/fundacao/ativa_desativa/' + id;
